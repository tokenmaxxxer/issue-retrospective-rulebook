...
#   1. Does this write reach reflect/state.md (by resolved target path)?
#      If so: is the resulting `stage` transition present as a row in
#      transition-rules.md? Present -> allow. Absent -> deny.
#   2. Does this write reach reflect's one owned kind (contract §11:
#      `reflect-record` at `docs/reports/records/<subject>/reflect.md`)
#      or a path structurally owned by another role (a
#      `docs/proposals/<date>-build-<slug>.md` slot, or a
...
#   3. If the write is a `reflect-record`
#      (`docs/reports/records/<subject>/reflect.md`): does it carry a
#      non-empty pointer to the other role records it read (coding-record,
#      qa-record, review-record, verify-record, etc.)? This is the
#      mechanically checkable half of contract §4's DEPENDS-ON bullet for
#      reflect (depends on the subject's other role records and any
#      finding blocks addressed to or from them). Missing -> deny. Per
#      contract §14, this check is intentionally shallow: it confirms the
#      pointer field is present and non-empty, not that the dependency's
#      substance — or the retro itself — is sound; what the retro
#      concludes stays reflect's own judgment, never this gate's.
...
#     reflect/state.md itself could not be read/parsed, or the hook input
#     was malformed)
...
#     reflect role does not own, per contract §11)
#   - "missing upstream-records pointer" (a reflect-record write has no
#     non-empty pointer back to the other role records it read, per
#     contract §4)
#
...
# into a computed name) is treated as reaching reflect/state.md ONLY when
# it is write-shaped toward reflect/state.md's directory; otherwise it is
...
deny() {
  echo "reflect-cycle: refused — $1" >&2
  exit 2
}
...
root = os.environ["REFLECT_ROOT"]
rules_path = os.environ["REFLECT_RULES_PATH"]
...
# reflect owns exactly one kind under this contract (§11): the
# `reflect-record` at `docs/reports/records/<subject>/reflect.md`.
# Unlike product (four owned kinds: hypothesis, product-record, one-pager,
# opportunity-tree), reflect's contract entry enforces structure only —
# one record slot per subject — and does not own any proposal-naming slot
# or standing doc. Also recognizes paths structurally owned by another role
# so a conflicting write can be refused-and-reported rather than silently
# overwritten or merged (contract §11).
...
def classify_path(rel_path):
    ...
    table. Returns (category, subject_or_None) where category is one of:
      "reflect-record"
        -> the one path reflect owns.
      "foreign"
        -> a path structurally owned by a different role (a
           `<date>-build-<slug>.md` or `<date>-<slug>.md` proposal, both of
           which belong to coding/product, not reflect; or a
           `docs/reports/records/<subject>/<other>.md` record where
           <other> is not "reflect").
      None
        -> outside this gate's owned-path/foreign-path concern entirely.
    """
    rel = rel_path.replace("\\", "/")
    if FOREIGN_BUILD_PROPOSAL_RE.match(rel) or FOREIGN_HYPOTHESIS_RE.match(rel):
        return "foreign", None
    m = RECORDS_RE.match(rel)
    if m:
        subject, record_name = m.group(1), m.group(2)
        if record_name == "reflect":
            return "reflect-record", subject
...
def looks_state_shaped(token):
    ...
    reflect/state.md (or reflect/'s directory), even though it cannot be
    ...
    t = token.lower()
    return "state.md" in t or re.search(r'(^|/)reflect(/|$)', t) is not None
...
def check_owned_path_write(rel_path, new_text):
    ...
    (reflect-record must point at the other role records it read). Denies on
    ...
    if category == "foreign":
        deny(
            ...
            "docs/specs/role-handoff-contract.md §11. reflect refuses to write there "
    ...
    if kind is not None and kind != category:
        deny(
            "path ownership conflict — %s is reflect's `%s` slot per contract §11, but the "
            "content self-declares `kind: %s`. Refusing rather than writing a mismatched "
            "kind into an owned path." % (rel_path, category, kind)
        )
    if category == "reflect-record":
        # Mechanically checkable half of contract §4's DEPENDS-ON: the
        # reflect-record must carry a non-empty pointer to the other role
        # records it read (coding-record, qa-record, review-record,
        # verify-record, etc.). This does NOT verify the pointer resolves
        # to real, correct records — per contract §14, kind is
        # self-declared and unverified, and this check only confirms the
        # field is present and non-empty.
        m = re.search(r'^(?:records_read|upstream_records):\s*(.+?)\s*$', new_text or "", re.M)
        if not m or not m.group(1).strip():
            deny(
                "missing upstream-records pointer — %s (a reflect-record) has no "
                "non-empty `records_read:` (or `upstream_records:`) field pointing at the "
                "other role records it read, per contract §4's DEPENDS-ON relationship." % rel_path
            )
    ...
    if isinstance(command, str) and command and _PRDD_TREE_RE.search(command):
        ...
        if not _prdd_plain_own_redirect_only:
            ...
            if not _prdd_proven_read_only:
                deny(
                    "reflect-cycle: refused — path-reference default-deny: this Bash command "
        ...
        for op_re in BASH_WRITE_OPS:
            for m in op_re.finditer(command):
                ...
                if cand is None:
                    ...
                    # else: not write-shaped toward reflect/state.md's
                ...
                if is_state:
                    reaches_state = True
                    continue
                # §11 ownership check for a Bash-mediated write to a literal
                # target that is not the state file (any idiom, including
                # write-through-another-tool like python3's open()).
