#!/usr/bin/env bash
# UserPromptSubmit hook: injects the current reflect-cycle state and the
# legal transitions out of it, read from transition-rules.md and
# reflect/state.md (frontmatter field `stage`).
#
...
fallback() {
  cat <<EOF
reflect-cycle: transition rules could not be loaded — $1
No transition of reflect/state.md's \`stage\` field may be made until this is fixed.
EOF
  exit 0
}
...
command -v python3 >/dev/null 2>&1 || fallback "python3 is not on PATH and is required to parse transition-rules.md and reflect/state.md."
...
state_path="$root/reflect/state.md"

REFLECT_RULES_PATH="$rules_path" REFLECT_STATE_PATH="$state_path" python3 <<'PY'
import os
import re
import sys

rules_path = os.environ["REFLECT_RULES_PATH"]
state_path = os.environ["REFLECT_STATE_PATH"]

def fail(reason):
    print("reflect-cycle: transition rules could not be loaded — %s" % reason)
    print("No transition of reflect/state.md's `stage` field may be made until this is fixed.")
    sys.exit(0)
...
if not os.path.exists(state_path):
    ...
    try:
        ...
        fail("reflect/state.md is missing or unreadable at %s (%s)." % (state_path, exc))

    stage_matches = re.findall(r'^stage:\s*(.*?)\s*$', state_text, re.M)
    if len(stage_matches) == 0:
        fail("reflect/state.md has no `stage:` field.")
    if len(stage_matches) > 1:
        fail("reflect/state.md has %d `stage:` fields; it must have exactly one." % len(stage_matches))
    current_stage = stage_matches[0].strip()
    if not current_stage:
        fail("reflect/state.md's `stage:` field is present but empty.")
    if current_stage not in known_states:
        fail("reflect/state.md's `stage:` value %r is not a known state." % current_stage)

# --- emit the block --------------------------------------------------------
applicable = [r for r in rows if r["from"] == current_stage]

print("reflect-cycle: current state — stage: %s" % current_stage)
if not applicable:
...
print(
    ...
    "and the model must append one line to reflect/state.md naming the user utterance "
