...
case "${ISSUE_RETROSPECTIVE_ACTION_ITEM_SHAPE_GATE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
try:
    ...
    try:
        ...
        deny("the tool-call payload is not valid JSON; the gate cannot judge action-item shape on an unparseable write.")
    if not isinstance(ev, dict):
        deny("the tool-call payload is not a JSON object; failing closed on action-item shape.")
