...
case "${ISSUE_RETROSPECTIVE_FREELUNCH_COMPLETENESS_GATE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
try:
    ...
    try:
        ...
        deny("the tool-call payload is not valid JSON; the gate cannot judge freelunch completeness on an unparseable write.")
    if not isinstance(ev, dict):
        deny("the tool-call payload is not a JSON object; failing closed on freelunch completeness.")
    ...
    # 3. Adopted-norms-with-rationale -- "adopted" co-occurring with
    #    rationale language.
    adopted_with_rationale = has_any("adopted") and has_any(
        "rationale", "per issue", "per the", "per approver", "because", "traced to"
    )
