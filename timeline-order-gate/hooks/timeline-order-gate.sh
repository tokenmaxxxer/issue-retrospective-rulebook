...
case "${ISSUE_RETROSPECTIVE_TIMELINE_ORDER_GATE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac

command -v python3 >/dev/null 2>&1 || deny "timeline-order-gate.sh requires python3, which is not on PATH; denying rather than guessing."

payload="$(cat 2>/dev/null || true)"
[ -n "$payload" ] || deny "timeline-order-gate: empty tool-use payload on stdin; cannot evaluate the gate."
...
[ -z "$root" ] && deny "no project root could be determined; failing closed (timeline-order check cannot run)."
...
try:
    ...
    try:
        ...
        deny("the tool-call payload is not valid JSON; the gate cannot judge timeline ordering on an unparseable write.")
    if not isinstance(ev, dict):
        deny("the tool-call payload is not a JSON object; failing closed on timeline ordering.")

    tool = ev.get("tool_name")
    ti = ev.get("tool_input")
    if not isinstance(ti, dict):
        deny("tool_input is missing or not a JSON object; the gate cannot judge a write it cannot parse (timeline ordering).")
    ...
    if os.path.isfile(r):
        try:
            ...
            deny("%s exists but cannot be read; failing closed on timeline ordering." % rel)
    ...
    if new_text is None:
        deny(
            ...
            "Edit/MultiEdit whose old_string matches, so timeline ordering can be "
            "checked." % (rel, tool)
        )
    ...
    before_timeline = new_text[: m.start()]
    ...
    _fc_sys.stderr.write("timeline-order-gate.sh: fail-closed: internal error: %r\n" % (_fc_e,))
    _fc_sys.exit(2)
PY
_fc_rc=$?  # fail-closed-on-internal-error
if [ "$_fc_rc" -ne 0 ] && [ "$_fc_rc" -ne 2 ]; then
  echo "timeline-order-gate.sh: fail-closed: internal error (judge exited $_fc_rc)" >&2
  exit 2
fi
exit "$_fc_rc"
